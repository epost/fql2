...
public abstract class SketchExporter extends Options {
	...
	/**
	 * The end-of-statement string, by default a ";". This will be set to the
	 * empty string when exporting directly to a db since queries are sent
	 * individually. Since ";" is only required in TEXT and FILE mode, not in
	 * DATABASE mode, <code>$</code> should always be appended to statements
	...
	/**
	 * The available export modes. Mode.TEXT is for output that will be returned
	 * as a single string; Mode.FILE is for writing to a file; Mode.DATABASE is
	 * for an export being sent directly to a db. Currently, the main different
	 * between them is that comments are suppressed from a db export.
	 */
	protected enum Mode {
		TEXT, FILE, DATABASE
	}
	...
	/**
	 * Returns the initialization string, which is a comment containing the
	 * export date and Easik name and URL.
	 *
	 * @return a comment
	 */
	 ...
	 * instance, for a db export), an empty list.
	 *
	 * @param text
	 *            to commentize
	 ...
	 * <p/>
	 * For example, db for a db driver. The db driver this export object was
	 * created from must support Database().
	 *
	 * @throws PersistenceDriver.LoadException
	 *             if the db driver cannot be loaded
	 */
	...
	protected class UniqueHM extends HashMap<EntityNode, LinkedList<HashSet<SketchEdge>>> {
		...
		/*
		 * Returns true if the EntityNode plus uniques auto-generated by the
		 * export already enforce uniqueness on the provided edges
		 */
		...
		/*
		 * Adds the specified SketchEdges as a new unique set associated with
		 * the EntityNode
		 */
