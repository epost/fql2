...
public class BinRelSet<X, Y> implements Iterable<Pair<X, Y>> {
	...
	public BinRelSet(BinRelSet<X, Y> r) {
		R = new LinkedHashMap<>();
		for (X x : r.R.keySet()) {
...
*/
	...
	public BinRelSet(Set<Pair<X, Y>> r) { R = r; }
	  
	  public BinRelSet() { R = new HashSet<>(); }
	  
	  public boolean add(X x, Y y) { return R.add(new Pair<>(x, y)); }
	  
	  public void remove(X x, Y y) { R.remove(new Pair<>(x, y)); }
	  
	  public <Z> BinRelSet<X, Z> compose(BinRelSet<Y, Z> rhs) { return new
	  BinRelSet<>(Util.compose(R, rhs.R)); }
	  
	  @Override public Iterator<Pair<X, Y>> iterator() { return R.iterator(); }
	  
	  public void addAll(BinRelSet<X, Y> a) { R.addAll(a.R); }
	  
	  public void removeIf(Predicate<Pair<X, Y>> p) { R.removeIf(p); }
