...
public class SqlMapping {
	...
	public static SqlMapping guess(SqlSchema source, SqlSchema target) {
		...
		Map<SqlTable, SqlTable> tm = new HashMap<>();
		Map<SqlColumn, Pair<SqlPath, SqlColumn>> am = new HashMap<>();

		for (SqlTable s : source.tables) {
			double max_d = -1;
			SqlTable max_t = null;
			for (SqlTable t : target.tables) {
				double cur_d = EditDistance.similarity(s.name, t.name);
				if (cur_d > max_d) {
					max_d = cur_d;
					max_t = t;
				}
			...
			for (SqlColumn c : s.columns) {
				...
				for (SqlColumn d : max_t.columns) {
					...
					double cur_d = EditDistance.similarity(c.name, d.name);
					if (cur_d > max_d) {
						max_d = cur_d;
						max_c = d;
					}
		...
		return new SqlMapping(source, target, tm, am, new HashMap<>());
	}
