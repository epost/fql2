...
public class Inferrer {
	...
	private static <Ty, En, Sym, Fk, Att, Gen, Sk> String inferInstance(Schema<Ty, En, Sym, Fk, Att> a) {
		...
		String eqs = "\tmulti_equations\n" + Util.sep(Util.alphabetical(a.fks.keySet()).stream().map(z -> "\t\t" + z 
				+ " /*:" + a.fks.get(z).first + "->" + a.fks.get(z).second + "*/ -> [list assignments here]").collect(Collectors.toList()), "\n");
		String eqs2 = "\t\n" + Util.sep(Util.alphabetical(a.atts.keySet()).stream().map(z -> "\t\t" + z 
				+ " /*:" + a.atts.get(z).first + "->" + a.atts.get(z).second + "*/ -> [list assignments here]").collect(Collectors.toList()), "\n");
