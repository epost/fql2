...
class Inferrer {
	...
	private static <Ty, En, Sym, Fk, Att> String inferInstance(Schema<Ty, En, Sym, Fk, Att> a) {
		...
		String ens = "\tgenerators\n" + Util.sep(Util.alphabetical(a.ens).stream().map
				(z -> "\t\t[list Generators here] : " + z).collect(Collectors.toList()), "\n");
		String eqs = "\tmulti_equations\n" + Util.sep(Util.alphabetical(a.fks.keySet()).stream().map(z -> "\t\t" + z 
				+ " -> [list assignments here] // " + a.fks.get(z).first + " -> " + a.fks.get(z).second).collect(Collectors.toList()), "\n");
		String eqs2 = "\t\n" + Util.sep(Util.alphabetical(a.atts.keySet()).stream().map(z -> "\t\t" + z 
				+ " -> [list assignments here] // " + a.atts.get(z).first + " -> " + a.atts.get(z).second).collect(Collectors.toList()), "\n");
	...
	private static <Ty, En, Sym, Fk, Att,Ty0, En0, Sym0, Fk0, Att0, Gen, Sk, Gen0, Sk0> String inferTransform(Instance<Ty, En, Sym, Fk, Att, Gen, Sk, ?, ?> a, Instance<Ty0, En0, Sym0, Fk0, Att0, Gen0, Sk0, ?, ?> b) {
		...
		String sks = Util.sep(a.sks().keySet().stream().
				...
				+ "] and labelled nulls [" + pr(b.sks().keySet()) + "]  ending at " + a.sks().get(z)).collect(Collectors.toList()), "\n");
	...
	private static <Ty, En, Sym, Fk, Att,Ty0, En0, Sym0, Fk0, Att0> String inferMapping(Schema<Ty, En, Sym, Fk, Att> a, Schema<Ty0, En0, Sym0, Fk0, Att0> b) {
		...
		String fks = "\tforeign_keys\n" + Util.sep(Util.alphabetical(a.fks.keySet()).stream().map(z -> "\t\t" + z 
				+ " -> a path of foreign keys [" + pr(b.fks.keySet()) + "] // " + a.fks.get(z).first + " -> " + a.fks.get(z).second).collect(Collectors.toList()), "\n");
		String atts = "\tattributes\n" + Util.sep(Util.alphabetical(a.atts.keySet()).stream().map(z -> "\t\t" + z 
				+ " -> lambda v. some type side symbols [" + pr(b.typeSide.syms.keySet()) + "] applied to Attributes [" + pr(b.atts.keySet()) + "] " + "applied to paths of foreign keys [" + pr(b.fks.keySet()) + "] ending on variable [v] " + " // " + a.atts.get(z).first + " -> " + a.atts.get(z).second + " ").collect(Collectors.toList()), "\n");
		return ens + "\n" + fks + "\n" + atts;
	}
	...
	private static <Ty, En, Sym, Fk, Att,Ty0, En0, Sym0, Fk0, Att0> String inferQuery(Schema<Ty, En, Sym, Fk, Att> a, Schema<Ty0, En0, Sym0, Fk0, Att0> b) {
		...
		String fks = "\tforeign_keys\n" + Util.sep(Util.alphabetical(b.fks.keySet()).stream().map(z -> "\t\t" + z 
				+ " [:" + b.fks.get(z).first + " ->" + b.fks.get(z).second + "] -> " + inferTrans(z, a, b)).collect(Collectors.toList()), "\n");
		return ens + "\n\n" + fks;
	...
	private static <Ty, En, Sym, Fk, Att,Ty0, En0, Sym0, Fk0, Att0> String inferTrans(Fk0 fk, Schema<Ty, En, Sym, Fk, Att> a, Schema<Ty0, En0, Sym0, Fk0, Att0> b) {
		...
		String gens = Util.sep(Util.alphabetical(dom).stream().map(z -> z + " -> a generator [" 
				+ pr(cod) + "] ." + " a path of foreign keys [" + pr3(a.fks.map) 
