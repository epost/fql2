...
public final class RawTerm {
	...
	Set<Triple<Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Ctx<Var, Chc<Ty, En>>, Chc<Ty, En>>> infer_good(
			...
			//	System.out.println("arg " + arg);
				...
				//	System.out.println("old " + old);
					...
					//	System.out.println("y " + y);
						...
						if (y.third.equals(Chc.inLeft(ty))) { //takes place of if (argt.equals(outcome.third)) {
		...
		if (col.fks.containsKey((Fk) e.head) && e.args.size() == 1 && e.annotation == null) {
			for (Triple<Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Ctx<Var, Chc<Ty, En>>, Chc<Ty, En>> outcome : infer_good(e.args.get(0), Chc.inRight(col.fks.get((Fk)e.head).first), col, pre, js, vars)) {
				Term<Ty, En, Sym, Fk, Att, Gen, Sk> ret1 = Term.Fk((Fk) e.head, outcome.first);
				Ctx<Var, Chc<Ty, En>> ret2 = new Ctx<>(outcome.second.map);
				Var v = new Var(e.args.get(0).head);
				Chc<Ty, En> ty = Chc.inRight(col.fks.get((Fk) e.head).first);
				if (vars.keySet().contains(v)) {
					if (ret2.containsKey(v) && !ret2.get(v).equals(ty)) {
						continue;
					} else if (!ret2.containsKey(v)) {
				...
				Chc<Ty, En> ret3 = Chc.inRight(col.fks.get((Fk) e.head).second);
				Chc<Ty, En> argt = Chc.inRight(col.fks.get((Fk) e.head).first);
		...
		if (col.atts.containsKey((Att) e.head) && e.args.size() == 1 && e.annotation == null) {
			//System.out.println("x " + e);
			for (Triple<Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Ctx<Var, Chc<Ty, En>>, Chc<Ty, En>> outcome : infer_good(e.args.get(0), Chc.inRight(col.atts.get((Att)e.head).first), col, pre, js, vars)) {
			//	System.out.println("y " + outcome);
				
				Term<Ty, En, Sym, Fk, Att, Gen, Sk> ret1 = Term.Att((Att) e.head, outcome.first);
				Ctx<Var, Chc<Ty, En>> ret2 = new Ctx<>(outcome.second.map);
				Var v = new Var(e.args.get(0).head);
				Chc<Ty, En> ty = Chc.inRight(col.atts.get((Att) e.head).first);
				if (vars.keySet().contains(v)) {
				...
				Chc<Ty, En> ret3 = Chc.inLeft(col.atts.get((Att) e.head).second);
				Chc<Ty, En> argt = Chc.inRight(col.atts.get((Att) e.head).first);
		...
		if (ret.isEmpty()) {
			...
			if (!vars.keySet().contains(new Var((String) e.head)) && !isSymbolAll(col,(String)e.head) && e.annotation == null) {
	...
	private static <Ty, En, Sym, Fk, Att, Gen, Sk> boolean isSymbol(Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col, String s) {
		return /* col.syms.map.containsKey(s) ||
				col.fks.map.containsKey(s) ||
				col.atts.map.containsKey(s) || */
				col.gens.map.containsKey(s) ||
				col.sks.map.containsKey(s);
	}
	
	private static <Ty, En, Sym, Fk, Att, Gen, Sk> boolean isSymbolAll(Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col, String s) {
		return  col.syms.map.containsKey(s) ||
				col.fks.map.containsKey(s) ||
				col.atts.map.containsKey(s) || 
				col.gens.map.containsKey(s) ||
	...
	Term<Ty, En, Sym, Fk, Att, Gen, Sk>,
	Chc<Ty, En>>	
	infer1x(Map<String, Chc<Ty, En>> ctx0, RawTerm e0, RawTerm f, Chc<Ty, En> expected,
			Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col, String pre, AqlJs<Ty, Sym> js) {
		Set<Quad<Ctx<Var, Chc<Ty, En>>, 
	Term<Ty, En, Sym, Fk, Att, Gen, Sk>, 
	Term<Ty, En, Sym, Fk, Att, Gen, Sk>,
	Chc<Ty, En>>	
	> ret = new HashSet<>();
		//Triple<Term<Ty,En,Sym,Fk,Att,Gen,Sk>,Ctx<Var,Chc<Ty,En>>,Chc<Ty,En>> lhs = infer_good(e, expected, col, pre, js, ctx0);	
		//Triple<Term<Ty,En,Sym,Fk,Att,Gen,Sk>,Ctx<Var,Chc<Ty,En>>,Chc<Ty,En>> rhs = infer_good(f, expected, col, pre, js, ctx0);	
		...
		Set<Triple<Term<Ty,En,Sym,Fk,Att,Gen,Sk>,Ctx<Var,Chc<Ty,En>>,Chc<Ty,En>>> 
		lhs = infer_good(e0, expected, col, pre, js, vars0);
		if (lhs.isEmpty()) {
			Util.anomaly();
		}
		Set<Triple<Term<Ty,En,Sym,Fk,Att,Gen,Sk>,Ctx<Var,Chc<Ty,En>>,Chc<Ty,En>>> 
		...
		if (ret.size() > 1) {
			...
			String msg = "Cannot infer a unique well-sorted term for " + e + ".\nCandidates: " + Util.sep(ret.stream().map(x -> f == null ? x.second.toStringUnambig() : x.second.toStringUnambig() + " = " + x.third.toStringUnambig()).collect(Collectors.toList()),"\n");
			if (expected != null) {
				msg += "Expected sort: " + expected.toStringMash() + " (isType=" + expected.left + ")";
			}
			throw new RuntimeException(pre + msg);
	...
	infer0x(
			Map<String, Chc<Ty, En>> ctx0, RawTerm e, Chc<Ty, En> expected,
			Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col, String pre, AqlJs<Ty, Sym> js) {
		...
		Set<Var> vars = ctx0.keySet().stream().map(x -> new Var((String)x)).collect(Collectors.toSet());
		Set<Triple<Term<Ty,En,Sym,Fk,Att,Gen,Sk>,Ctx<Var,Chc<Ty,En>>,Chc<Ty,En>>> 
		ret = infer_good(e, expected, col, pre, js, vars0);
		if (ret.isEmpty()) {
			Util.anomaly();
		}
		Ctx<Var, Chc<Ty, En>> initial = new Ctx<>();
		...
		for (Triple<Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Ctx<Var, Chc<Ty, En>>, Chc<Ty, En>> outcome : ret) {
			if (!vars.containsAll(outcome.second.keySet())) {
				Util.anomaly();
			}
		}
		//System.out.println("a " + ret.size() + ret);
		ret.removeIf(x -> !initial.agreeOnOverlap(x.second)); //updates in place
		System.out.println("b " + ret.size() + ret);
		ret.removeIf(x -> !Util.union(x.second.keySet(),initial.keySet()).equals(vars));
	...
	infer0x(Map<String, Chc<Ty, En>> ctx0, RawTerm lhs, Chc<Ty, En> expected,
		...
		Term<Ty, En, Sym, Fk, Att, Gen, Sk>, 
		Term<Ty, En, Sym, Fk, Att, Gen, Sk>,
		Chc<Ty, En>>	 ret = infer1x(ctx0, lhs, null, expected, col, pre, js);
		
		return new Triple<>(ret.second, ret.first, ret.fourth);
	}*/

	 /* public static <Ty, En, Sym, Fk, Att, Gen, Sk> Term<Ty, En, Sym, Fk, Att, Gen, Sk> infer0(
			Map<String, Chc<Ty, En>> ctx0, RawTerm lhs, Chc<Ty, En> expected,
			Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col, String pre, AqlJs<Ty, Sym> js) {
		...
		try {
			...
			Triple<Ctx<String, Chc<Ty, En>>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>> ret = infer1(
					ctx, new RawTerm(fresh, Collections.emptyList()), lhs, col, js);
			...
			if (!actual.equals(expected)) {
				throw new RuntimeException("in " + lhs + ", infered sort is " + actual.toStringMash()
	...
	// each prim
	/*public static <Ty, En, Sym, Fk, Att, Gen, Sk> Triple<Ctx<String, Chc<Ty, En>>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>> infer1(
			Map<String, Chc<Ty, En>> ctx, RawTerm lhs, RawTerm rhs, Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col,
			AqlJs<Ty, Sym> js) {
		...
		if (!lhs_t.x.equals(rhs_t.x)) {
			throw new RuntimeException("For LHS " + lhs + ", the infered sort of RHS " + rhs + " is "
					+ lhs_t.x.toStringMash() + " but " + rhs + " actually has sort " + rhs_t.x.toStringMash());
		}
		Ctx<String, Chc<Ty, En>> ret = new Ctx<>();
		for (String var : ctx0.keySet()) {
			Ref<Chc<Ty, En>> ref = ctx0.get(var);
			if (ref.x == null) {
				throw new RuntimeException("In equation " + lhs + " = " + rhs + ", the variable/primitive " + var
						+ ", has ambiguous sort");
			}
...
/*
	private <Ty, En, Sym, Fk, Att, Gen, Sk> Term<Ty, En, Sym, Fk, Att, Gen, Sk> trans(Set<String> vars,
		...
		int n = boolToInt(vars.contains(head)) + boolToInt(col.syms.containsKey(resolve(head)))
				+ boolToInt(col.atts.containsKey(resolve(head))) + boolToInt(col.fks.containsKey(resolve(head)))
				+ boolToInt(col.gens.containsKey(resolve(head))) + boolToInt(col.sks.containsKey(resolve(head)));
		if (n > 1) {
			throw new RuntimeException(
					head + " is ambiguously a variable/function/attribute/foreign key/generator/lablled null");
		}
		...
		if (annotation != null) {
			...
			throw new RuntimeException("In term " + this
			...
			if (!col.java_tys.containsKey(ty) && col.java_parsers.containsKey(ty)) {
				throw new RuntimeException(
						"In term " + this + ", " + ty + " has a java parser but is not declared as a java type");
			}
			if (col.java_tys.containsKey(ty) && !col.java_parsers.containsKey(ty)) {
				throw new RuntimeException(
						"In term " + this + ", " + ty + " is a java type but does not hava a java parser");
			}
			if (!col.java_tys.containsKey(ty) && !col.java_parsers.containsKey(ty)) {
...
/*
	...
	private <Ty, En, Sym, Fk, Att, Gen, Sk> Ref<Chc<Ty, En>> infer(Set<String> vars, Map<String, Ref<Chc<Ty, En>>> ctx,
		...
		int sum = Util.list(isSym, isAtt, isFk, isGen, isSk, isVar, isObj).stream().filter(x -> x.equals(Boolean.TRUE))
				.collect(Collectors.toList()).size();
		if (sum > 2) {
			throw new RuntimeException(
					"In " + this + ", the head " + head + " is ambiguous: " + (isSym ? "is a typeside symbol " : " ")
							+ (isVar ? "is a variable " : " ") + (isFk ? "is a foreign key " : " ")
							+ (isAtt ? "is an attribute " : " ") + (isGen ? "is a generator " : " ")
							+ (isSk ? "is a labelled null " : " ") + (isObj ? "is a primitive" : ""));
		}
		...
		if (isGen) {
			...
			if (args.size() != 1) {
				throw new RuntimeException("In " + this + ", the head " + head
			...
			if (arg_t.x != null && !Chc.inRight(ty).equals(arg_t.x)) {
				throw new RuntimeException(
						"In " + this + ", the head " + head + " is an attribute/foreign key expecting argument type "
								+ ty + " but its argument has actual type " + arg_t.x.toStringMash());
			}
			...
			if (args.size() != syms_t.first.size()) {
				throw new RuntimeException("In " + this + ", the head " + head + " is a typeside symbol of arity "
			...
			for (RawTerm arg : args) {
				...
				if (arg_t.x != null && !Chc.inLeft(syms_t.first.get(i)).equals(arg_t.x)) {
					throw new RuntimeException("In " + this + ", the head " + head + " at position " + i
							+ " is expecting argument type " + syms_t.first.get(i)
							+ " but its argument has actual type " + arg_t.x.toStringMash());
				}
				arg_t.set(Chc.inLeft(syms_t.first.get(i))); // redundant
															// sometimes
				i++;
			}
	...
	public static <X, Y, Z> RawTerm fold(Set<X> fks, Set<Y> entities, List<Z> l, String v) {
		...
		for (Object o : l) {
			if (entities.contains(o)) {
				if (fks.contains(o)) {
					//throw new RuntimeException("In " + Util.sep(l, ".") + ", " + o
						//	+ "is ambiguous: it can refer to an entity (0-length path) or a foreign key (1-length path)");
				} else {
					continue;
				}
			} 
