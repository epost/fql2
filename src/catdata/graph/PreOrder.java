...
public class PreOrder<X> {
	...
	private static <X> Set<PreOrder<X>> allTotal2(Collection<X> xs) {
		...
		/*
	public static <X> Set<PreOrder<X>> allTotal(Collection<X> xs) {
		List<Pair<X, X>> dom = new LinkedList<>();
		List<Boolean> cod = new LinkedList<>(); 
		cod.add(true);
		cod.add(false);
		...
		Set<PreOrder<X>> ret = new HashSet<>();
		List<LinkedHashMap<Pair<X,X>, Boolean>> cands = FinSet.homomorphs(dom, cod);
		for (Map<Pair<X, X>, Boolean> cand : cands) {
			PreOrder<X> p = new PreOrder<>();
			for (Pair<X,X> pair : cand.keySet()) {
				if (cand.get(pair)) {
					p.add(pair.first, pair.second);
				} else {
					p.add(pair.second, pair.first);
				}
			}
			ret.add(p);
		}
