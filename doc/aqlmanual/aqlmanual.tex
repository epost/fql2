...
\subsection{{\tt options}}
Allowed are {\tt timeout} and {\tt dont\_validate\_unsafe}.  Note that each instance and transform in the query can have its own options (the Query Evaluation Entity Block options described in this manual).
...
\section{Query Evaluation Entity Block options}
...
Sets whether or not it is permissible for AQL to choose a join order based on a heuristic.
