...
\section{HTML markup}
The AQL IDE allows HTML markup to be inserted into AQL files.  After a program has compiled, the ``emit as HTML'' option in the AQL menu will emit the HTML code.  The HTML will contain pretty-printings of the program text, HTML tables for instances, and javascript graphs for schemas.  To insert HTML, use the keyword {\tt html}, then brackets, and then a quoted string (note that because the HTML appears in a string, any quotes and backslashes within that string must be escaped).  For example,
\begin{verbatim}
instance I = ...
...
Due to a bug in the code editor, html sections will not fold if the closing quote is on the same line as the closing bracket.  The AQL tutorial, built-in as the Tutorial example, illustrates HTML output.
...
A {\it kind} is either {\it typeside}, {\it schema},  {\it instance}, (schema) {\it  mapping}, (instance) {\it transform}, {\it query}, {\it graph}, or {\it pragma}. 
...
%todo
Composes two mappings: if {\tt f : A -> B} and {\tt g : B -> C}, then {\tt compose f g : A -> C}.
...
\section{{\tt compose <transform> <transform>}}
%todo
Composes two transforms: if {\tt f: A -> B} and {\tt g: B -> C}, then {\tt compose f g: A -> C}.
