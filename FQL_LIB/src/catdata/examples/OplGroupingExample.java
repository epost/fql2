...
public class OplGroupingExample extends Example {
	...
	String s = "S0 = theory { "
			...
			+ "\n	symbols zero -> \"return 0\","
			+ "\n		   succ -> \"return input[0]+1\","
			+ "\n	        worksInID -> \"var util=Java.type('fql_lib.Util'); "
			+ "\n	                      var chc=Java.type('catdata.algs.Chc');"
			+ "\n	                      var term=Java.type('fql_lib.opl.OplExp.OplTerm');"
			+ "\n	                      var wrap=Java.type('fql_lib.opl.OplExp.JSWrapper');"
			+ "\n	                      var revd = util.rev0(I.saturate().symbols.get('deptID'));"
			+ "\n	                      var cand =  new term(chc.inRight(new wrap(input[0])), []);"
			+ "\n	          	       var dept =  revd.get(cand)[0];    "
			+ "\n	          	       var w = util.revS(I.saturate().symbols.get('worksIn'));"
			+ "\n	          	       var u = w.get(dept); "
