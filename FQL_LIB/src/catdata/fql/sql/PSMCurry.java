...
//								Object Ifx = lookup(HcJ.first.evaluate(f.arr), x);
	...
	public void exec(PSMInterp interp,
		...
		try {
			...
			for (Node c : sig.nodes) {
				Quad<Instance, 
				     Map<Node, Map<Object, Transform>>, 
				     Map<Node, Triple<Instance, Map<Object, Pair<Object, Object>>, Map<Pair<Object, Object>, Object>>>, 
				     Pair<Map<Node, Triple<Instance, Map<Object, Path>, Map<Path, Object>>>, 
				          Map<Edge, Transform>>> kkk = interp.exps.get(inst_dst);
				Instance HcJ = kkk.third.get(c).first;
				//System.out.println("for " + c + ": " + kkk.third.get(c));
				...
				for (Pair<Object, Object> xx : I.data.get(c.string)) {
					...
					for (Node d : sig.nodes) {
						...
						for (Arr<Node, Path> f : sig.toCategory2().first.hom(c, d)) {
							...
							//	System.out.println(v);
								Object pathX = kkk.fourth.first.get(c).third.get(f.arr);
								Object iii = kkk.third.get(c).third.get(new Pair<>(y.first, pathX));
					...
					Transform xxx = new Transform(HcJ, K, tx); //Hc * J -> K
					Object yyy = lookup2(kkk.second.get(c), xxx);
					s.add(new Pair<>(x, yyy));
				}
				l.add(new Pair<>(c.string, s));
			}
