...
public class Driver {
	...
	public static Env makeEnv(String str,
			...
			Program<Exp> last_prog, Env last_env) {
		Env env = new Env();
	...
	//	}
		...
		if (last_str != null /* && NEWDEBUG.debug.opl.opl_cache_gui */) { //TODO
			for (String k : init.order) {
				...
				Exp a = init.exps.get(k);
				Exp b = last_prog.exps.get(k);
				if (!a.equals(b)) {
		...
		for (String n : init.order) {
			...
			try {
				...
				t.printStackTrace();
				throw new LineException(t.getLocalizedMessage(), n, "");
			}
		}
