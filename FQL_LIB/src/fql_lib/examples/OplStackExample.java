...
public class OplStackExample extends Example {
	...
	String s = "//requires disabling mapping check"
			...
			+ "\n		plus : E,E -> E;"
			+ "\n 	equations"
			+ "\n 		forall x:E. plus(z(), x) = x,"
			+ "\n 		forall x:E, y:E. plus(s(x), y) = s(plus(x,y));"
			+ "\n //		forall x:E, y:E. plus(x,y) = plus(y,x);"
			+ "\n}"
			+ "\n"
			+ "\nCSem = javascript {"
			+ "\n	symbols"
			...
			+ "\n		append : S,S -> S;"
			+ "\n	equations"
			+ "\n		forall x:S. append(empty(), x) = x,"
			+ "\n		forall x:S. append(x, empty()) = x,"
			+ "\n		//etc"
			+ "\n"
			+ "\n		/* equations below should be provable: */"
			+ "\n		"
			+ "\n		//forall x:E. plus(z(), x) = x"
			+ "\n		forall x:S. plus(append(push(empty(), z()), x)) = x, "
			+ "\n		"
			+ "\n 		//forall x:E, y:E. plus(s(x), y) = s(plus(x,y))"
			+ "\n 		forall x:S, y:S. plus(append(plus(append(x, push(empty(), i()))), y)) "
