...
import fql_lib.pp.CatOps;
import fql_lib.pp.FQLPPEnvironment;
import fql_lib.pp.FQLPPProgram;
import fql_lib.pp.FunctorExp;
		...
		if (sig != null && !view.isInfinite()) {
			if (DEBUG.debug.cat_schema) {
				...
				if (g.getVertexCount() == 0) {
					...
					CodeTextPanel xxx = new CodeTextPanel(BorderFactory.createEtchedBorder(), "", "Graph has " + g.getVertexCount() + " nodes, which exceeds limit of " + DEBUG.debug.MAX_NODES);
					px.add("Schema", xxx);
				} else if (g.getEdgeCount() > DEBUG.debug.MAX_EDGES) {
					CodeTextPanel xxx = new CodeTextPanel(BorderFactory.createEtchedBorder(), "", "Graph has " + g.getEdgeCount() + " edges, which exceeds limit of " + DEBUG.debug.MAX_EDGES);
					px.add("Schema", xxx);
				}
				else {
...
//		JPanel gp = new JPanel(new GridLayout(2, 3));
	...
	public Display(String title, FQLPPProgram p, FQLPPEnvironment env) {
		this.prog = p;
		this.env = env;
	//	this.gui = gui;
